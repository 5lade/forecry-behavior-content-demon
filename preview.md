# Content Engine Preview

**Flavor:** Content Demon  
**Outcome:** Turns notes into posts, pages and launch assets  
**Default mode:** `draft`  
**Hosted check cost:** 2 credits

## Before install sample

> Content Engine triggered because it found a useful opportunity: turns notes into posts, pages and launch assets.

## Intensity examples

- **3/10:** occasional suggestions — suggestion-only unless asked.
- **6/10:** daily/active support — drafts visible artifacts and logs cost.
- **10/10:** hourly attempts when safe and budgeted — still respects quiet hours, spend caps, and approvals.

## Landing snippet

Turn messy notes into launch-ready posts, pages, and emails — with approval notes before anything goes public.

## Marketplace loop

Preview is free. Install syncs the signed manifest. Refinement and hosted private pack compile are paid events. Uninstall removes local behavior config and can revoke the API key.

## Stacking/conflict copy

Content Engine yields when another Behavior has safer or higher-priority ownership, then records the handoff in the activity log.
